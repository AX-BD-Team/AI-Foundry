...
import { fetchQualityMetrics } from "@/api/analytics";
...
export function ProjectStatusTab() {
  ...
  useEffect(() => {
    void fetchQualityMetrics(organizationId)
      .then((res) => {
        if (res.success) setQuality(res.data);
        else toast.error("품질 지표 로드 실패");
      })
      .catch(() => toast.error("품질 지표 API 호출 실패"))
  ...
  const totalDocs = quality?.parsing.totalDocuments ?? 0;
  const approvedPolicies = quality?.policy.approvedCount ?? 0;
  const totalTerms = quality?.skill.totalTerms ?? 0;
  const totalSkills = quality?.skill.totalSkills ?? 0;
  const approvalRate = quality?.policy.approvalRate ?? 0;
  const avgTrust = quality?.policy.avgTrustScore ?? 0;
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <MetricCard icon={FileText} label="투입 문서" count={totalDocs} color="#3b82f6" />
          ...
          <div className="relative">
            ...
            {totalTerms === 0 && (
              <Badge className="absolute top-2 right-2 text-[0.6rem]" variant="outline">집계중</Badge>
            )}
          </div>
          <div className="relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
          ...
          <div className="p-3 rounded-lg border" style={{ borderColor: "var(--border)" }}>
            <div className="text-xs" style={{ color: "var(--text-secondary)" }}>평균 신뢰도</div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="p-4 rounded-lg border" style={{ borderColor: "var(--border)" }}>
            ...
            <div className="text-[0.65rem]" style={{ color: "var(--text-secondary)" }}>855건 문서 처리 기준</div>
          </div>
          <div className="p-4 rounded-lg border" style={{ borderColor: "var(--border)" }}>
            <div className="flex items-center gap-2 mb-1">
